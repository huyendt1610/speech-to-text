...
class LibrispeechDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if (is_from_cached and cache_version > 0 and os.path.exists(os.path.join(cached_path, str(cache_version)))): 
            ...
            self.audio2mels = T.MelSpectrogram( # default n_fft & hanning_window
                ...
                n_mels=n_mels # Mel filter banks: (optimal value) Output sẽ có 80 hàng (80 tần số mel), => chia trục tần số thành 80 dải Mel.
    ...
    def __getitem__(self, index):
        ...
        # print(mel.shape) # => [1, 80, 853]: 80 bins, each time step (25s) has 80 values according to each mel bins, magnetide is the value at each mel bin
        # print(mel[0][:, 0])
...
def collate_fun(batch): # combine multiple samples into batchs
    ...
    # batch_first = True => return B*T*..., else T*B*...
    ...
    # print(spectrograms.shape) # [5, 1, 853, 80] # add 1 channel dimension for CNN, because CNN expect input with 4 dimensions: batch_size, num_channels, height, width => here num_channels = 1 (because we have only 1 spectrogram), height = 80 (number of mel bins), width = 853 (number of time steps)

    spectrograms = spectrograms.transpose(-1, -2) # switch the position of 2 last dimensions  # [5, 1, 80, 853]
